...
\subsubsection{Activity profile of procedures}
...
at that step. For the measure of activity $\mathcal{A}$ we can simply take the norm of the
appropriate derivative, as it measures the rate of change. This measure can be
easily replaced by a more sophisticated one.
   ...
   $$\mathcal{A}_i\circ T_i\circ T_{i-1}\circ\dots T_1(v)$$ 
   is the \emph{activity level} of $T_i$, for a given element of the domain $v\in \Omega$ taken as the input of $T$.
   ...
   \For{each $T_i$}
   \State extract $a^i_k=\mathcal{A}_i\circ T_i\circ T_{i-1}\circ\dots T_1(v_k)$
	\EndFor
	\EndFor
   ...
   Once the activity profiles and property measures are generated, starting with an element, without the property $X_j$, we can use any optimization techniques, to optimize and increase the measure $M_{X_j}$. When the increase of the measure $M_{X_j}$ is sufficient, this results in an new object possessing the property $X_j$.
   ...
   \State compute the energy $E_i=e^{\D_v}_1(M_{X_j})\circ T^\prime_i$
   ...
   If we take the measure of activity to be the norm of the derivative 
   $$\left\Vert\D_vT^i_1\right\Vert=\left\Vert proj_{\{\D\}}\left(e^\D_1(T^i_1)\right)\right\Vert$$
   
   Than for each property $X_j$ we select the set of sub-procedures 
   $$T_{X_j}=\{T_i\in T_{X_j}\iff \left\Vert\D_vT^i_1\right\Vert\ge c\}$$
   that have the highest measure of activity at the elements of $X_j$. The property measure for the property $X_j$ is than simply the sum of squares of norms of the derivatives, of selected sub-procedures
   
   \begin{equation}
   M_{X_j}=\sum\limits_{T_i\in T_{X_j}} \left\Vert\D_vT^i_1\right\Vert^2.
   \end{equation}
   
   If $T\in\dP_0$ represents a neural network, where $T_i$ stands for a specific neuron in the system, than the procedure given by Algorithms 1, 2, is equivalent to Google's Deep Dream Project. However our procedure may be applied to any procedure $T\in\dP_0$, no only neurl networks. Our view is broader, as neural networks are just programs contained in the programming space $\dP_0$.
