...
\section{Analysis}\label{sec:Analysis}
  ...
  allows deduction of whether $v$ has the property $X$ or not. Let $S$ be a
  property of the image of $T$ such that 
  
     \begin{equation}\label{eq:equivRelation}
     v\in X\iff T(v)\in S
     \end{equation}
For example $S(v)$ might be $|1-T(v)| < c$.
Using the map $T$ together with the property $S$ one can determine wheather an
element $v$ has the property $X$.     
...
\subsubsection{Conceptual steps of importance}
...
at each step. For the measure of activity $M$ we can simply take the norm of the
appropriate derivative, as it measures the rate of change. This measure can be
easily replaced by a more sophisticated one.
   Let split the procedure $T$ into conceptual steps $T_i$:
$$T=T_n\circ T_{n-1}\circ\cdots\circ T_1.\footnote{for simplicity we assume
  there are no control structures in $T$, but we could just as easily take $T$
  to be a spline as in \eqref{eq:zlrprk_splosno} in Section
  \ref{sec:control}}$$ Each  $T_i$ might be a complex program itself.
...
The algorithm $A$ is a broad generalization of special cases such as Google's
Deep Dream project \cite{DeepDream}, where the program $T$ represents a neural network, with neurons with high activity filling the set of important conceptual steps $T_{X_j}$ and the resulting $v\in X_j$ is an altered image.
