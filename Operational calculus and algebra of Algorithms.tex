...
\begin{definicija} A \emph{Euclidean computing machine} is a pair $(\VV,\E)$, such
...
\begin{definicija}[Automatically differentiable programs]
  ...
  \end{equation}
The Euclidean computing machine $(\VV, \E)$ is \emph{automatically differentiable}
...
Let $\dP_n$ be the subspace, spanned by $\cup_{k\le n}\D^k \dP_0$ over $K$.
 ...
 \begin{izrek}\label{izr:P_n}
 	Function space $\dP_n:V\to U\otimes T(V^*)$ is isomorphic to the tensor
 ...
 \begin{proof} Follows directly from the proof of Theorems $\ref{izr:P}$ and $\ref{izr:dP}$ through argument $\eqref{eq:inductionStep}$.
  \end{proof}
By taking the limit as $n\to \infty$, we set
...
\end{equation}
for each $n\ge 1$, where $e^\D_n$ is the projection of the operator onto the set $\{\D^n\}$.
\end{izrek}      
