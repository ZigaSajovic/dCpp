...
of its dual. Virtual memory serves by its self as an algebra of polynomial
...
constructs a procedure that enforces that property upon any object. Through it we generalize state of the art methods for analyzing neural networks to general programs.
...
The paper offers new insights into programs, as we demonstrate how to probe their inner structure, revealing what actions properties of the domain most react to.
...
a set of elementary operations that are sequentially applied into programs. 
\begin{definicija} A \emph{Euclidean computing machine} is a pair $\langle \VV,\dP\rangle$, such
  that 
  \begin{itemize}
  \item
  $\VV$ is a finite dimensional vector space over a filed $K$ serving as alphabet symbols
  \item
  $\dP < \VV^\VV$ is a subspace of maps $P:\VV\to \VV$, called \emph{programming space} serving as states
...
(\VV\otimes\VV^*)+\ldots.$$
We will call $\VV_\infty$ the \emph{universal virtual memory space} of an
Euclidean computing machine. The term virtual memory is used as it is
...
Thus $\D^k$ is a mapping between function spaces $\eqref{eq:F^n}$
 ...
 \begin{definicija}\label{def:dP}
 	A differentiable programming space $\dP_0$ is any subspace of $\F_0$ such that
...
$$\D\dP_0\subset\dP_0\otimes T(V^*)$$
  ...
  $\D^k\dP_0\subset\dP_0\otimes T(V^*)$ and $0$, where
  $\alpha$ is multiindeks denotind the component of $T(V^*)$ and $i$ is the
  indeks denoting the component of $U$.
...
The tuple $M=\langle \VV,\dP_0\rangle$ is an analytic, infinitely  differentiable programming language.
   
    \begin{itemize}
    \item
    $\VV$ is a finite dimensional vector space
    \item
    $\VV\otimes T(\VV^*)$ virtual memory space, serving as alphabet symbols
    \item
    $\dP_0$ is a differentiable programming space over $\VV$, such that all elements of $\dP_0$ are analytic
    \end{itemize}
  \end{definicija}
...
\end{algorithmic}
\end{algorithm}
Thus we get $I=\{T_{X_i}\}$, where $T_{X_i}$ is the set of steps $T_j$ measured as important by Algorithm $\ref{alg:partitioning}$. Each set $T_{X_j}$ contains conceptual steps, portions of the source code, whose action on the input contributes most in a partitioning to the equivalence class $X_j$. As such, the set $I$ shows to be useful in studying properties of the set $M$, as it reveals to what action, a property of an element $v\in M$ reacts to, as we probe it using the method. The perturbations reveal internal structure of te procedure.
