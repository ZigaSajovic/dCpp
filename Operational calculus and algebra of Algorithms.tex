...
\begin{definicija}[Euclidian virtual machine] The tuple $\langle \VV,\F\rangle$ is a Euclidian virtual machine.
  \begin{itemize}
  \item
...
By taking the limit as $n\to \infty$, we see
  ...
  \end{proof}
    ...
    The following
     ...
     \end{equation}
   	holds by Theorem $\ref{izr:P_n}$, allowing simple implementation, where only explicit knowledge of the operator $\sumd_1$ is required, as dictated by expression $\eqref{eq:potenca(1+d)}$.
