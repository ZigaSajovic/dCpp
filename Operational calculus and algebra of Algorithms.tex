...
constructs a procedure that imposes that property upon any object. Through it we generalize state of the art methods for analyzing neural networks to general programs and expand upon them.
...
We generalize state of the art methods for analyzing neural networks \cite{DeepDream} to general programs and provide a framework for analysis of machine learning architectures and other virtual constructs, as actions on the virtual space.
...
Thus $\D^k$ is a mapping between function spaces $\eqref{eq:F^n}$
 ...
 \begin{trditev}\label{izr:linearnaNeodvisnost}
  The set $\{\partial^k;\quad 0\le k\le n\}$ represents linearly independent vectors over a field $K$, spanning the subspace $\mathcal{D}^n$.
 \end{trditev}
...
\begin{izrek}\label{izr:P}
	Any programming language $\dP_0$ satisfying Definition $\ref{def:dP}$ is an
...
Let $\dP_n$ be the subspace, spanned by $\cup_{k\le n}\D^k \dP_0$ over $K$.
 \end{definicija}
...
By taking the limit as $n\to \infty$, we see
  ...
  \end{proof}
    ...
    The following
     \begin{equation}
     	\sumd_n(P\in\dP_0)\in\dP_0\otimes T_n(V^*)
     \end{equation}
...
We motivate the following with an example. Suppose an algorithm of high time complexity. As speed is of importance, an approximation would suffice. Using the described mechanics, we may linearize the program. We choose a $v_0\in\dP$ and linearize $P'=P(v_0)+\D P(v_0)\cdot(v-v_0)$. The latter has to be done only once.
 ...
 Locally, the image of the linearization is a good approximation of the original program. To extend this radius, we would like to expand said program into a series. By Claim $\ref{izr:linearnaNeodvisnost}$  there exists a space that is spanned by the set $\{\D^k\}$ over a field $K$. Thus, the expression
 \begin{equation}
...
\left. \left((\D^n e^{h\D})(P)(\vv_0)\right)(\vv)\right|_{h=0}$$
 ...
 Theorem $\ref{izr:P_n}$ implies
     \begin{equation}
     	e^{h\D}(P\in\dP)\in\dP_0\otimes T(V^*)
     \end{equation}      
...
\end{trditev}      
 Thus, we gained the ability of writing a differentiable program $P_1$ acting on derivatives of another program $P_2$, stressed as crucial (but lacking in most models) by other authors \cite{AD1}. Usage of the reduction of order map is demonstrated in Section \ref{sec:Analysis}.
 ...
 The problem is on the edge of domains $\partial\Omega_i$, where the program $P$ may not be differentiable. A problem which may be avoided, with methods of the coming sections.
  ...
  By differentiating $\eqref{eq:kh}$ at the fixed point, we get at the constant $\lambda$.
  ...
  Methods offer a different perspective on the halting problem, where it can be studied through convergence and fixed points.
...
\section{Analysis}\label{sec:Analysis}
  ...
  To achieve this, we first develop a way of probing the inner structure of a procedure and explore how it interacts with the elements of the domain. Let $R$ be an equivalence relation 
...
\State extract $T^\prime=\phi\circ e^{\D_{T_i}}_2(T^n_j)$ $\eqref{eq:T'}$
\State compute the energy $E_j=e^{\D_v}_1(C)\circ T^\prime_j$
\State extract the derivative $\D_v E=proj_{\{\D\}}(E_j)$
...
\begin{equation}
\D_vE_j=proj_{\{\D\}}(e^{\D_v}_1(C)\circ T^\prime_j)
\end{equation}
...
\subsection{Computational capability as shape governing principles}

As each procedure is revealed to be a system existing as an object in virtual space, its computational capability becomes synonymous with shape. For example, training of neural network is a homotopy in this space, as a high-dimensional manifold converges to its optimal computational form.
The developed calculus allows as to sculpt and mold a procedures shape, and through the established connection, its computational capability. There is a strong connection between shape of the inhabiting space and physical principles it imposes upon objects traversing it.

As procedures are systems entered by an object as an input and exited as an output upon undergoing change, the process of computation can be viewed as evolutions in phase space. Thus, our newfound ability to mold shape in virtual space enables us to impose principles on the system the procedure represents. Assume we have some a priori knowledge of how the process is to change the object, some previously known heuristic $H$ known from practice. That is equivalent to having knowledge on the path to change $\gamma$, as argued in the previous section. Heuristic $H$ might be some measure on the environment, or a direction, for objects at a particular location $L_i$ in the system. We express change the desired heuristic $H$ locally exerts, with a relation.
Thus, $H$ represents a local principle we wish to impose into the system. We denote the fact the system obeys the principle $H$ by $P\in H$. The desire is an algorithm molding the system, modifying it in some way, as to impose the principle upon it.
...
\end{equation}
be the procedure representing the system and $P_i$ the conceptual step treated by heuristic $H_j$. $L_j$ are the concerning location. Again, we let $P^i_k=P_i\circ\cdots\circ P_k$. Directions are usually modeled by first derivatives, and attractions by second derivatives, so lets assume employment of both by the heuristic $H$. For simplicity, assume $step$ to represent one optimization step, using a first derivative in its' execution. A method easily enhanced by any of more sophisticated optimization techniques readily available.
