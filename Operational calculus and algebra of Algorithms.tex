    ...
    The following
     ...
     \end{equation}
   	holds by Theorem $\ref{izr:P_n}$, allowing simple implementation as dictated by expression $\eqref{eq:potenca(1+d)}$.
        ...
        $$\dP_0\to\dP_n.$$
 \begin{corollary}
   When all elements of $\dP_0$ are analytic, so are the elements of $\dP_n$.
   \end{corollary}        
...
The tuple $M=\langle \VV,\dP_0\rangle$ is an analytic, infinitely  differentiable virtual machine.
    ...
    \item
    $\dP_0$ is an analyticz programming space over $\VV$
    \end{itemize}
  \end{definicija}
Anything that operates under these specifications is an analytic virtual machine, with the ability to implement infinitely differentiable programming spaces $\dP_\infty$. When not all elements of $\dP_0$ are analytic, the above defines an infinitely-differentiable virtual machine.
