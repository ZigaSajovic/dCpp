...
Thus the theory opens new doors in program analysis. A method of probing procedures, studying their internal structure and input archetypes is developed. We show the existence of a procedure testing an object for a property to be sufficient for constructing a procedure imposing that property upon it. 
\end{abstract}
...
The way evaluation of algorithmic expressions differs from evaluation of symbolic expressions of standard analysis, lies at the very core of this dissonance. The disconnect was partially mitigated by methods of automatic differentiation. Yet under the lack of a proper formalism the model collapses when one tries to generalize to such notions as a differentiable program $p_1$ operating on derivatives of another program $p_2$, where only coding of $p_2$ is required within $p_1$ (as opposed to coding the derivative of $p_2$ itself), while retaining the desired freedom of expression. 
Models allowing for nested transformations \cite{AD1}, still fail in providing algorithms with an algebra enabling study and formulation of programs through analytic equations. Thus, they remain nothing more than means to calculating derivatives, lacking the true power the rest of the vast field of analysis is endowed with.
