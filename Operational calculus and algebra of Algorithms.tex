...
quotient of tensor algebra $T(V^*)$. To any element of
 ...
 \end{equation}
Applying contraction multiple times with the same vector $\vv$ one can assign a
monomial map to any simple tensor $\uu\otimes
 f_1\otimes\ldots\otimes f_n\in U\otimes(V^*)^{\otimes n}$
 \begin{equation}
   \label{eq:tensor->poly}
 \uu\otimes f_1\otimes\ldots\otimes f_n: \vv \mapsto f_n(\vv)\cdots f_1(\vv) \uu
 \end{equation}

that can be extended by linearity to all multi-tensors in $U\otimes T(V^*)$.
