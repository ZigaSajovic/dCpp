...
% \begin{equation}
% e^{h\D}(f\circ g)=exp(\D_fe^{h\D_g})(f,g)
% \end{equation}
% \end{izrek}
% 
%\begin{proof}
...
% $$\implies$$
% $$e^\D(f\circ g)=e^{\D_f}\sum\limits_{\forall_n}(h\D_g)^n\sum\limits_{\lambda(n)}\prod\limits_{k\cdot l\in\lambda}(\frac{\D_f}{l!})^k\frac{1}{k!}$$
% kjer $\lambda(n)$ ozačuje particije $n$.
% $$\lim\limits_{\lVert h\rVert\to 0}(\frac{d}{dh})^n$$
% \end{proof}       
% 
