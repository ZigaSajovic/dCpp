...
\begin{definicija}
  ...
  S tem se obrnemo h konstrukciji algebre nad $V_n$. Vsota po komponentah je
  ...
  \end{lstlisting}      
   ...
   Pred nadaljevajem predpostavimo obstoj sledeče funkcije
    ...
    ki v $\mathcal{O}(n\log(n))$ zlije dve $\textit{mapi}$, tako da pare ujemajočih se indeksov preslika s $func: V^{\times n}\to V$, ostale pa prekopira. Skupaj z operacijama po komponentah imamo sedaj linearne kombinacije $var\in V_n$ nad poljem $K$. Tenzorski produkt je implicitno definiran z $var$.
      ...
      Po Izreku $\ref{izr:tauRek}$ lahko rekurzivno definiramo preslikave $\sumd_{n+1}=1+\D\sumd_n$, ki jih po Izreku $\ref{izr:P_n}$ generiramo z algebro nad $\dP_n$.\clearpage
...
Po Izreku $\ref{izr:D^nRek}$ velja  $\D^k(\mathrm{f_1}\circ \mathrm{f_2})=\D^{k-1}(\D \mathrm{f_1}\circ\D \mathrm{f_2})$, kar se izraža kot zaporedje skalarnih produktov. Formulo se zlahka implementira skozi $\textit{breadth-first queue}$ linearnih preslikav s praznimi preseki domen.   
