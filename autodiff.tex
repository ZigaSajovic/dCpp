...
\begin{definicija}
  ...
  Za zgled uporabe teorije predstavljamo konstrukcijo analitičnega stroja, z zmožnostjo implementacije neskončno odvedljive različice programskega jezika $\CC$ nad virtualnim pomnilnikom $V\simeq\RR$.
  ...
  Pred nadaljevajem predpostavimo obstoj sledeče funkcije generične paradigme
  \begin{lstlisting}
  template<class var_1, class var_2, class mapping>
  OutputIterT merge_apply(
      var_1 first, var_2 last, var_3 output, Func func);
  \end{lstlisting}
  
  ki v $\mathcal{O}(n\log(n))$ zlije dve $\textit{mapi}$, tako da pare ujemajočih se indeksov preslika s $func:V\times V\to V$, ostale pa prekopira. Sedaj se obrnemo h konstrukciji algebre nad $V_n$.
  
  \begin{lstlisting}
    template<class T>
    T sum_pairs(T a, T b) {
      return std::make_pair(a.first, a.second.reduceOrder() + b.second.reduceOrder());
    }
    \end{lstlisting}
  
  Funkcija služi kot vsota po komponentah na univerzalnem virtalnem prostoru $V_n\simeq V\otimes T(V^*)$, ki skupaj z moženjem s skalarjem
  
  \begin{lstlisting}
  template<class T>
  T mul_make_pair(var_1 a, var_2 b) {
    return std::make_pair(a.first, a.second.reduceOrder() *b);
  }
  \end{lstlisting}
  
  omogoča generiranje linearnih kombinacij. Z objektom $dTau$ modeliramo multitenzor $\D\sumd_{n-1}(\dP)$. Očitno velja $\sumd_n\dP=id+dTau$. Generična narava nam bo omogočila konstrukcijo večih operacij.
  
  Po Izreku $\ref{izr:D^nRek}$ lahko rekurzivno definiramo preslikave $\D^{n+1}(\mathrm{p}_2,\circ\mathrm{p}_1)=\D^n(\D\mathrm{p}_2\circ\D\mathrm{p}_1)$, za vsako komponento $\D^n\dP:V\to V_n$ posebej. Tako je konstrukcija diferenciala $\sumd_n\dP:V\to V_n$ preprosta, saj po Izreku $\ref{izr:dP}$ velja $\sumd_n\in\dP_0\otimes T(V^*)$.
  ...
  class tau
  {
      public:
          tau();
          tau(funDouble mapping, tau primitive);
          ~tau();
          var operator()(const var&v_1, const var&v_2);
      private:
          tau primitive_v_1;
          tau primitive_v_2;
          funDouble mapping;
  ...
  Po Izreku $\ref{izr:tauRek}$ lahko rekurzivno definiramo preslikave $\sumd_{n+1}\dP=(1+\D\sumd_n)\dP$, ki jih po Izreku $\ref{izr:P_n}$ generiramo z algebro nad $\dP_n$. Diferencial $\sumd$
  
  \begin{lstlisting}
   var tau::operator()(const var&v_1, const var&v_2=var(0)){    
        var out;
        out.real=this.mapping(v_1.real, v_2.real);
        if(v_1.order>0||v_2.order>0){
        	std::map<int,double> tmp_1;
            std::map<int,double> tmp_2;
       	 	for_each_copy(v_1.dual,v_2.dual,tmp_1, this.primitive_v_1, v_2.reduceOrder());
       	 	for_each_copy(v_2.dual,v_1.dual,tmp_2, this.primitive_v_2, v_1.reduceOrder());
      	 	 merge_apply(tmp_1,tmp_2, out, sum_pairs<pair<int, double> >);
      	}
        return out;
    }
  \end{lstlisting}
  
  Naj bo programski jezik generiran z množico
    
    $$\dP=\{+(v_1,v_2),\cdot(v_1,v_2),^{\huge{\wedge}}(v_1,v_2)\sin(v), \cos(v),e^v,\ln(v)\}$$
    
    Operacije, kot denimo razlika, $-$, in deljenje, $\div$, so vsebovane v $\dP$, skozi linearne kombinacije $p\in\dP$.
