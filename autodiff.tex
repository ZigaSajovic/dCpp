...
% $\mathcal{K}$ kontrolne strukture
 ...
 %notacije je treba dodelat niso še čisto kul
...
\end{equation}
Potem je razvoj v odrezano vrsto moč izrazit kot skrčitev
...
Nadaljevanje motiviramo s primerom. Predpostavimo fizično strojno opremo, ki implementira stroj $M$.
...
\begin{equation}\label{eq:innerProd}
	\int_{\Omega}\langle\phi_1,\phi_2\rangle(V) d\textbf{v}
\end{equation}
kjer se razume, da $d\textbf{v}$ predstavlja mero $dv_1dv_2...dv_n$, kjer je $n$ dimenzija domene. Lastnosti notranjega produkta in integrala zagotvaljajo, da je $\eqref{eq:innerProd}$ notranji produkt nad $L^2(\Omega,V)$. Očitno je $\eqref{eq:tenV}$ (ob poenostavitvi $\eqref{eq:tenEn}$) element $\Phi$.
