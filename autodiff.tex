...
\begin{proof}
 ...
 \end{proof}       
 \begin{opomba}
 Zato je včasih smiselno predhodno aplicirat $g(v)\in V$ na $f$, saj $e^\D$ apliciran na konstanto predstavlja identiteto in tako dobimo odvod v točki $v\in V$ namesto razvoja v multitenzor.
  $$\sum\limits_{\lambda(n)}\prod\limits_{k\cdot l\in\lambda}\Big(\frac{\D_f\D_g^l(g)}{l!}\Big)^k\frac{n!}{k!}\Big(\Big(e^{\D_f}\Big)f(g(v))\Big)$$
  $$\implies$$
  \begin{equation}\label{eq:dCompositePoint}
  \sum\limits_{\lambda(n)}\prod\limits_{k\cdot l\in\lambda}\Big(\frac{\D_f\D_g^l(g)}{l!}\Big)^k\frac{n!}{k!}\Big(f(g(v))\Big)
  \end{equation}
 \end{opomba}
