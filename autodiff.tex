...
\frac{d}{dh}e^{h\D_g}=\D_ge^{h\D_g}
 ...
 \begin{equation}\label{eq:Dplus}
  ...
  V kolikor $F$ ne napenja $\dP$, oziroma smo vzeli projekcijo operatorja $e^\D_{n<N}$, izraz $P_F$ kljub temz predstavlja najboljši približek prvotnega programa $P$ na $n$ komponentah $\{\D^n\}$.
  
  Pred izračunom tenzorja $\eqref{eq:matTrans}$ je smiselno nabor funkcij $F$ razširiti z medsebojnimi kompozicijami in s tem pridobiti funkcije, ki niso izrazljive skozi linearne kombinacije, in tako povečat moč metode.
