...
Računanje višjih odvodov preslkave $P:V\to U$, lahko sedaj predstavimo z eno samo
preslikavo $\sumd_k:V\to U\otimes T(V^*)$:
\begin{eqnarray}
  \label{eq:vsi_odvodi}
...
$S_k(V^*)$ se ne spreminja, ne glede na to kakšna je dimenzija domene in ko-domene za vmesne operacije. Na ta način zmanjšamo potrebo po spominu na minimum.
\end{opomba}
...
Tako je $\D^k$ preslikava med funkcijskimi prostori $\eqref{eq:F^n}$
 ...
 \end{proof}
 Po Izreku $\ref{izr:linearnaNeodvisnost}$ lahko edinstveno definiramo geometrijsko vrsto
 ...
 \begin{equation}\label{eq:DD}
  ...
  Tako je $\sumd_n$ preslikava med funkcijskimi prostori $\eqref{eq:F_n}$
