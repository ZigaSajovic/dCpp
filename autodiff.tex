...
\begin{proof}
 ...
 Zato je včasih smiselno predhodno aplicirat $g(v)\in V$ na $f$, saj $e^\D$ apliciran na konstanto predstavlja identiteto in tako dobimo odvod $\D^n$ v točki $v\in V$ namesto razvoja v multitenzor.
  ...
  \end{equation}
 \end{opomba}
 Izrek $\ref{izr:kompo}$ omgogoča enostavno implementacijo komponiranja programov $p\in\dP$ izraženih v odrezanih vrstah skozi izraz $\eqref{eq:dComposite}$. 
...
\frac{d}{dh}e^{h\D_g}=\D_ge^{h\D_g}
 ...
 \begin{equation}\label{eq:Dplus}
  ...
  \subsection{Funkcijske transformacije programov}
  ...
  V kolikor se želimo izognit integriranju, je skozi razvita orodja problem rešljiv z linearno algebro. Z $e_n^\D$ označnimo projekcijo operatorja $e^\D$, na prvih $n$ baznih vektorjev $\{\D^i\}$. Po Izreku $\ref{izr:e^d}$ lako skozi operator $e^\D$ konstruiramo preslikavo $\eqref{eq:pToPol}$ iz prostora programov $\dP$ v prostor polinomov z nedoločenkami v  $V^i$. Z $\X$ označimo bazo funkcijskega prostora polinomov, $\X=\{\prod\limits_{\forall_j} v_i^j\}$, kjer $v_i$ napenjajo $V$ in $j$ teče po elementih potenčne indeksne množice, invariantno permutacijam (kar je ekvialentno bazi prostora tenzorskega produkta, kjer tenzorski produkt nadomesti navaden produkt, ki seveda komutira). Potem lahko $e^\D_n(P\in\dP)$ interpretiramo kot vektor linearnih kombinacij $\X$, kar je privzeto v nadaljevanju poglavja.
  
  S tem lahko definiramo tenzor transformacij baze $F\to\X$
  
  \begin{equation}\label{eq:matTransF}
  A_{\X F}=\begin{pmatrix}
  e_n^\D(f_1) & e_n^\D(f_2) & \cdots & e_n^\D(p_n)
  \end{pmatrix}
  \end{equation}
  ...
  Tako je tenzor transformacije baze $\X\to F$ enaka
  
  \begin{equation}\label{eq:matTrans}
  A_{F\X}=A_{\X F}^{-1}
  \end{equation}
  
  Skozi tenzor $\eqref{eq:matTrans}$ zlahka izvedemo tranformacijo baze $\X\to F$. Tako so koordinate programa $P\in\dP$ v bazi $F$
  
  \begin{equation}\label{eq:P_F}
  	P_F=A_{F\X}\cdot e^\D(P)
  \end{equation}
  ...
  V kolikor $F$ ne napenja $\dP$, oziroma smo vzeli projekcijo operatorja $e^\D_{n<N}$, izraz $P_F$ predstavlja najboljši približek prvotnega programa $P$ na $n$ komponentah $\{\D^n\}$.
