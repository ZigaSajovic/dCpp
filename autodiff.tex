...
$$\mathcal{V}=\mathcal{M}\bigoplus_{\forall_i}\partial_i\mathcal{M}$$
  ...
  \begin{equation}\label{eq:dirSumV}
    \Op: V\to V\bigoplus (V^*\bigotimes V)
    \end{equation}
...
% $\mathcal{K}$ kontrolne strukture
 ...
 oziroma v poznani, konceptualno ekvivalentni notaciji
  ...
  \begin{izrek}\label{izr:dM}
	$\Op M=\langle\Op\T, \Op V, \Op\rangle$ je odvedljiv računski stroj.
  \end{izrek}
  Dokaz je trivialen, raširjen operator $\circ$ je dobro definiran, tako je $(\Op\T, \circ)$ monoid in $\Op V\subset V_\infty$.
 ...
 Izrek $\ref{izr:dM}$ dovoljuje rekurzivno konstrukcijo potence operatorja, ki je definirana
 ...
 Dimenzija prostora $\op{n} V$ je $\frac{1-k^{n+1}}{1-k}$, kjer je k dimenzija $V$.
 \end{izrek}
 ...
 \begin{equation}\label{eq:vrsta}
 	\VV^i +\VV^i_j(v^j-v_0^j)+\frac{1}{\alpha_2!}\VV^i_{jk}(v^{jk}-v_0^{jk})+\frac{1}{\alpha_3!}\VV^i_{jkl}(v^{jkl}-v_0^{jkl})+...+\frac{1}{\alpha_k!}\VV^i_\JJ(v^\JJ-v_0^\JJ)
 \end{equation}
kjer se razume, da $v^{ij..n}$ predstavlja $v^iv^j...v^n$.
V nadaljenjem zaradi berljivosti predpostavimo centiranost okoli ničle. Podobno lahko definiramo $V^{\JJ_k}_{k}\subset T(V)$ 
...
\end{equation}
Slika skrčitve je element prvotnega virtualnega prostora $V$. Neodvisnost $(\ref{eq:tenzorVrsta})$ od koordinatnega sistema, se v programiranju prevede na neodvisnost v izvajanju. Tako je izraz $(\ref{eq:tenzorVrsta})$ invarianten stopnji izvajanja programa. 

Za konstrukcijo $\eqref{eq:tenzor}$ potrebujemo en $v\subset V$, zato $\eqref{eq:tenV}$ poenostavimo v
\begin{equation}\label{eq:tenEn}
	\VV^{ik}_{\JJ_k}:V^k\to\bigoplus\limits_{0\le j\le k} V^{\otimes j}\to V^i
\end{equation}
kjer $k$ in $i$ ozačujeta dimenziji podprostorov zanimanja. Potem je $\eqref{eq:tenV}$ element komutativno asociativne unitalne algebre $S(V^*)$, kar korespondira polinomom z nedoločenkami v $V$. Ob ustrezni izbiri domene je $\VV^{ik}_{\JJ_k}\in L^2$.

Nadaljevanje motiviramo s primerom. Predpostavimo fizično strojno opremo, ki implementira stroj $M$, ki je optimizirana za izvajanje nabora funkcij $F=\{f_1,f_2,...,f_n\}$, ki jih navaja proizvajalec. Situacija, ki je pogosta v proizvodnji GPU in CPU.

Predpostavimo obstoj programa $P\subset \T$, izvajan na omenjeni strojni opremi. Kot je v praksi pogosto, smo za hitrost pripravljeni žrtvovat optimalnost in se zadovoljimo s suboptimalnim programom. Zato bi želeli program $P$ izrazit v bazi $F$. Zaradi berljivost predpostavimo, da so $F$ ortogonalizirali z eno od mnogih obstoječih metod.

Predpostavimo, da je osnovni prostor $V$ $\mathbb{H}ilbertov$ z notranjim produktom $\langle\cdot ,\cdot \rangle$. Potem lahko definiramo notranji produkt nad $\Phi=\{\phi: V\to V\}$ kot

\begin{equation}\label{eq:innerProd}
	\int_{\Omega}\langle\phi_1(v),\phi_2(v)\rangle d\textbf{v}
\end{equation}
kjer se razume, da $d\textbf{v}$ predstavlja mero $dv^1dv^2...dv^n$, kjer je $n$ dimenzija domene. Lastnosti notranjega produkta in integrala zagotvaljajo, da je $\eqref{eq:innerProd}$ notranji produkt nad $L^2(\Omega,V)$. Očitno je $\eqref{eq:tenV}$ (ob poenostavitvi $\eqref{eq:tenEn}$) element $\Phi$, kot so tudi $f_i\subset F$.
