...
\frac{d}{dh}e^{h\D_g}=\D_ge^{h\D_g}
 ...
 Kot je razvidno iz primera, je računanje z operatorji precej enostavneje od direktnega manipuliranja funkcij. Seveda posledično omogoča enostavnejšo implementacijo, ki deluje nad poljubnimi funkcijskimi prostori. V prostoru ki ga $\{\D^n\}$ napenja nad $\dP$, denimo lahko odvode kompozicij izrazimo skozi pravila produkta Izreka $\ref{izr:prod}$ in $\eqref{eq:dexp}$, skozi operatorje same. Tako je eksplicitno poznanje pravil odvajanja kompozituma nepotrebno, saj je vsebovano v sami strukturi operatorja $exp(\D_fe^{h\D_g})$, katerega pa odvajamo po klasičnih pravilih, kot v zgornjem primeru.
