...
\begin{definicija}
  ...
  ki v $\mathcal{O}(n\log(n))$ zlije dve $\textit{mapi}$, tako da pare ujemajočih se indeksov preslika s $func: V^{\times n}\to V$, ostale pa prekopira. Sedaj se obrnemo h konstrukciji algebre nad $V_n$.
  ...
  Funkcija služi kot vsota po komponentah na univerzalnem virtalnem prostoru $V_n\simeq V\otimes T(V^*)$, ki skupaj z moženjem s skalarjem (upoštevajoč kompozicijo).
  ...
  class tau
  {
      public:
          tau();
          tau(funDouble mapping, Vector<funVar> dVars);
          ~tau();
  ...
  Po Izreku $\ref{izr:tauRek}$ lahko rekurzivno definiramo preslikave $\sumd_{n+1}\dP=(1+\D\sumd_n)\dP$, ki jih po Izreku $\ref{izr:P_n}$ generiramo z algebro nad $\dP_n$.
  
  \begin{lstlisting}
     var tau::operator()(Vector<var&>vars){    
        var out;
        out.real=this.mapping(vars.id);
        if(min_order(vars)>0){
  ...
  S tem smo konstruirali tudi funkcijsko algebro na prostoru programov, ki jo napenja $\{\D^n\}$ nad $\RR$ in je po Izreku $\ref{izr:P_n}$ izomorfizem $\dP\infty\simeq\dP_0\otimes T(V^*)$. Velja,
  
  $$tau:\dP^{\times n}\to \dP_\infty$$
  $$tau:V\times\dP^{\times n}\to V_\infty$$
  
  Posledično obstaja algebra nad $V_\infty$ generirana s $tau$:
    ...
    \begin{lstlisting}
   	var var:: operator+(double n)const;
        ...
        var var:: operator-(const var& v)const;
    \end{lstlisting}
  Za jasnost, enega od elementov konstruiramo še eksplicitno, brez $tau$.
  
  \begin{lstlisting}
    var var::operator^(double n) const{
    	var out;
        out.real=std::pow(this->real,n);
        for_each_copy(this->dual, out.dual, mul_make_pair<&var, var> >, *this, n*(this->reduceOrder())^{n-1});
