...
kjer velja posplošeno Leibnitzevo pravilo

\begin{equation}
p=fg\implies\sum\limits_{i+j=k}\D^if\D^jg=\D^kp
\end{equation}
$$\implies$$
\begin{equation}\label{eq:P_Prod}
\sumd_n f\sumd_n g=\sumd_n(fg)
\end{equation}

Tako je denimo produkt nad $\dP_2$ za $f,g\in\dP$

$$(f+\D f+\D^2 f)(g+\D g+\D^2 g)=fg+f\D g+g\D f+\D f\D g + f\D^2g+g\D^2 g$$

S tem je definicija kompozicije preslikav $\dP_n$ preprosta

\begin{equation}
\sumd_{n+1}(f\circ g)=f\circ g + \D\sumd_nf\cdot\D\sumd_ng
\end{equation}

kjer za jasnost $\cdot$ predstavlja produkt $\eqref{eq:P_Prod}$.        
...
\subsection{Optimizacija prostorske zahtevnosti}
 ...
 Po Izreku $\ref{izr:linearnaNeodvisnost}$  obstaja prostor $\dP_n$, ki ga množica $\{\D^k\}$ napenja nad $\RR$. Zato je izraz   
  ...
  \begin{equation}
  	\frac{\D}{1-e^{h\D}}=\sum\limits_{n=0}^{\infty}c_n\frac{(h\D)^n}{n!}
  \end{equation}
