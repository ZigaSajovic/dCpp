...
\begin{opomba}
  ...
  Definicija se v svojem bistvu ne bi spremenila, le da bi bila polna
  tehnikalij, ki se jim bomo tu raje izognili. 
\end{opomba}
...
% $\mathcal{K}$ kontrolne strukture
 ...
 %\Op V je prostor v katerem se dogaja, \Op\T so preslikave/operatorji nad tem prostorom
	 \Dplus M=\langle\Dplus\T, V\oplus (V^*\otimes V), d\rangle
 \end{equation}
 ...
 \subsection{Univerzalni virtualni pomnilnik}
...
\begin{equation}
	\Dplus^n=\sum\limits_{n=0}^{n}\sum_{\forall_{i,..,k}}\frac{\partial^n }{\partial x_i...\partial x_k}dx_i\otimes dx_j\otimes...\otimes dx_k
\end{equation}
 in slika $\E\to\sum\limits_{i=0}^nd^i\E$. Rekurzivna defincija odraža strukturo prostora $\subseteq_{\forall_i}\Dplus^i\T$. Algebra je dobro definirana. Izračun pokaže
 \begin{equation}
 \Dplus^n=\frac{1- d^{n+1}}{1- d}
 \end{equation}
 Tako so preslikave, ki operirajo nad univerzalnim vektorskim prostorom $V_n$ definirane kot slike operatorja $\Dplus^n$. Operator apliciramo na $\T$.
 \begin{equation}
 	\Dplus^n:\T\to \T^{n'}
 \end{equation}
