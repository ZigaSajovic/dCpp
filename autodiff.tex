...
\end{equation}
saj so gre za zaključen izraz $\eqref{eq:t^n}$; če bi nad izrazom dalje izvajali potisk/povlek, bi slikali $V^i_n\to\VV^{ik}_{\JJ_k}\in V^i_n$.
...
ki je upravičena z opažanjem, da za vsak $n$ in $i$ velja
$$\lim\limits_{\lVert v\rVert\to 0}\frac{\partial^n }{\partial v_i...\partial v_n}\text{(leva stran)}=\lim\limits_{\lVert v\rVert\to 0}\frac{\partial^n }{\partial v_i...\partial v_n}\text{(desna stran)}$$
kjer se razume, da je $v^{\JJ_k}_{k}$ generiran z enim $v\in V^i$.

Slika skrčitve je element prvotnega virtualnega prostora $V^i$. Neodvisnost $(\ref{eq:tenzorVrsta})$ od koordinatnega sistema, se v programiranju prevede na neodvisnost v izvajanju. Tako je izraz $(\ref{eq:tenzorVrsta})$ invarianten stopnji izvajanja programa. 
