...
% $\mathcal{K}$ kontrolne strukture
 \end{itemize}
 %na te mestu dodati navodila konstrukcije odvodov višjega reda, skozi rekurzivno definicjo, kjer dualu prišemo svoj dual
 
 Karkoli operira pod predstavljenimi specifikacijami, je odvedljiv Turingov stroj.
 
 Z ustrezno izbiro projekcije $\mathcal{P}_\II$ programa, kot $\mathcal{M}\subset V$ omogočimo preučevanje različnih lastnosti programa. Projeciramo lahko na enotske vektorje, ki predstavljajo konceptualni vhod programa in proučujemo spremembe prvega reda. S tem merimo odzivnost programa na vhodne podatke.
 
 
  Večina algoritmov v uporabi, vsebuje hiper-parametre, katerih natančne vrednosti so odvisne od specifičnosti aplikacije. Skozi projekcijo na $\mathcal{H}\subset V$, kjer $\mathcal{H}$ predstavljaja hiper-parametre, algoritem interpretiramo, kot parametrično družino mnogoterosti. Uravnavanje teh hiper-parametrov, ki je navadno prepuščeno izkušnji programerja, postane ekvivalentno optimizaciji funkcij v prostoru parametrov.
  Tako so ob konstrukciji algoritmov skozi $M$\ref{eq:dTuring}, vsi algortimi, ki vsebujejo hiper-parametre, naučljivi.
