...
smiselno ignorirati in si misliti, da ostale enake.
...
Tako je $\D^k$ preslikava med funkcijskimi prostori $\eqref{eq:F^n}$
 ...
 \begin{izrek}\label{izr:D^nRek}
 	Operator $\D^k$ je rekuzivno definiran z verižnim pravilom:
...
\begin{proof}
 ...
 $$\D^n(f\circ g)$$
 $\impliedby$
 $$exp(\D_fe^{h\D_g})=e^{\D_f}\prod_{i=1}^{\infty}e^{\D_f\frac{(h\D_g)^i}{i!}}$$
 $$\implies$$
 $$exp(\D_fe^{h\D_g})(g)(f)=e^{\D_f}\sum\limits_{\forall_n}h^n\sum\limits_{\lambda(n)}\prod\limits_{k\cdot l\in\lambda}\Big(\frac{\D_f\D_g^l(g)}{l!}\Big)^k\frac{1}{k!}\Big(f\Big)$$
 kjer $\lambda(n)$ ozačuje particije $n$.
 $$\lim\limits_{\lVert h\rVert\to 0}(\frac{d}{dh})^nexp(\D_fe^{h\D_g})=\lim\limits_{\lVert h\rVert\to 0}e^{\D_f}\sum\limits_{\forall_m}n!h^{n-m}\sum\limits_{\lambda(m)}\prod\limits_{k\cdot l\in\lambda}\Big(\frac{\D_f\D_g^l(g)}{l!}\Big)^k\frac{1}{k!}\Big(f\Big)$$
 $$\implies$$
 \begin{equation}\label{eq:dComposite}
 e^{\D_f}\sum\limits_{\lambda(n)}\prod\limits_{k\cdot l\in\lambda}\Big(\frac{\D_f\D_g^l(g)}{l!}\Big)^k\frac{n!}{k!}\Big(f\Big)
 \end{equation}
 kjer $e^{\D_f}$ opravi shift v specifično točk, saj je za ocenitev v točki ekvivalenten identiteti $e^{\D_f}(v\in V)=v$.
 \end{proof}       
 
 Izraz $\eqref{eq:dComposite}$ je analog Faà di Bruno-vi formuli.
