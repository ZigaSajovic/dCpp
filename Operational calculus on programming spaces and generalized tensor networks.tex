...
By taking the limit as $n\to \infty$, we will consider 
 	...
 	\begin{equation}
 	\label{eq:P_algebra}
 	 	    \dP_\infty < \dP_0\otimes \overline{T(\VV^*)} = \dP_0 \otimes\left(\prod_{k=0}^\infty (\VV^*)^{\otimes k} \right),
 	\end{equation}
where $\overline{T(\VV^*)}=\prod_{k=0}^\infty (\VV^*)^{\otimes k}$ is the space
of the  infinite formal tensor series,
