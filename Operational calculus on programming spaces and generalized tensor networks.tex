...
We generalize neural networks by constructing general tensor networks in Section \ref{sec:generalTensorNet} and reveal their connection with programs in Section \ref{sec:progAsNet}. In Section \ref{sec:transToNet} we derive transformations of arbitrary programs to general tensor networks, providing a meaningful way of network initialization. These tensor networks are freely composed with general programs, as revealed in Section \ref{sec:compoNetProg}. Such constructs are trainable (as is their training process itself) and adhere to the operational calculus presented in this paper. This is demonstrated in Section \ref{sec:trainNet}, enabling analytic study through the theory.
...
To any element of
 ...
 namely a polynomial map  $\VV\to \VV$. Thus, similarly to \eqref{eq:P_algebra}, we consider the completion of the symmetric algebra $S(\VV^*)$ as the \emph{formal power series} $\Ss(\VV^{*})$, which is in turn a quotient of \emph{tensor series algebra} $\T(\VV^*)$, arriving at 
...
Let $W\in \VV\otimes T_n(\VV^*)$ be an element of the virtual memory. We define the application of $v\in \VV$ by
\begin{equation}
...
By an algorithm similar to Algorithm \ref{alg:appoint}, we can drive a procedure $P$, whose conceptual step $P_i$ maximizes action of the principle $H$ upon the input objects inhabiting the virtual memory.
