...
\epigraph{The enchanting charms of this sublime science reveal themselves in all their beauty only to those who have the courage to go deeply into it.}{--- \textup{Karl F. Gauss}}
...
We generalize neural networks by constructing general tensor networks in Section \ref{sec:generalTensorNet}. In Section \ref{sec:transToNet} we derive transformations of arbitrary programs to general tensor networks, providing a new way of network initialization. These tensor networks are trainable and adhere to the operational calculus presented in this paper as demonstrated in Section \ref{sec:trainNet}, enabling analytic study through the theory.
...
\left. \left((\D^n e^{h\D})(P)(\vv_0)\right)(\vv)\right|_{h=0}$$
 ...
 \end{proof}
 \begin{corollary}
 The operator $e^{h\D}:\dP\times V\to U\otimes T(V^*)$ evaluated at $h=1$ is a broad generalization of the shift operator \cite{OpCalc}, as the theory it exists in offers more than a mere shift, as will become apparent in the coming sections.
...
\exp(\D_fe^{h\D_g}): \dP\times\dP\to\dP_\infty,
 ...
 When projecting onto the space spanned by $\{1,\D\}$, the action of the operator $\exp(\D_fe^{h\D_g})(g)$ of $\eqref{eq:opKompo}$ is equivalent to forward mode automatic differentiation. Both forward and reverse mode are obtainable using the same operator $\exp(\D_fe^{h\D_g})$ of $\eqref{eq:opGenKompo}$, by fixing the appropriate one of the two maps. This generalizes both concepts under a single operator in the theory. The \eqref{eq:kompo} can be generalized for the notion of a pullback, to arbitrary operators.
 \end{trditev}
...
\item
$W_i\in \bigoplus\limits_{i=0}^n\VV\otimes \VV^{*\otimes i}$ are the weights (and the bias)
\item
...
Methods of practical use presented in the previous Section \ref{sec:TransInPractice} apply to general tensor networks. Thus, they may freely be composed with and included in other (differentiable) programs, transformations and tensor networks.
...
\begin{trditev}
Using the operator $\exp(\D_fe^{h\D_g})$ of Theorem \ref{izr:kompo}, both forward and reverse mode automatic differentiation (of arbitrary order) can be implemented on general tensor networks, as by Claim \ref{trd:reverseForward}.
\end{trditev}
