...
\begin{definicija}[Differentiable programs]
  ...
  \end{equation}
\end{definicija}
\section{Polynomial approximations and tensor algebra}
In order to apply the definition of derivative of a program to higher
derivatives, we use the tensor algebra.
Let $P:V\to U$ be a map on vector spaces. We define universal codomain to be the
spaces
\begin{eqnarray}
  \label{eq:universal_space}
  U_\infty &=& U\otimes\left(F\oplus V^*\oplus (V^*)^{\otimes 2}\oplus
               \ldots \right)\\
 U_\infty &=& U\otimes T(V^*)
\end{eqnarray}
