...
\maketitle
\begin{abstract}
In this paper, we develop the theory of analytic machines, which implements infinitely-differentiable programs, and operators acting upon them.

A programming language is defined as a monoid in a functional vector space of mappings of the virtual memory. Virtual memory is constructed through tensor algebra and endowed with the algebra of algorithms. Using this algebra, we construct differential operators, that span the space of infinitely differentiable programs. We present a wholesome theory of operators, that enables analysis of programs and computations on the operator level, which favors general implementation. Through the theory, we derive new operators, that expand a program into infinite tensor series in the algebra of algorithms. The theory is used to analyze iterators and other control structures in programming languages, and their dependencies on boundary conditions.

The developed facilitates functional transformations of programs in an arbitrary functional basis (usually the one specified as optimal by the hardware manufacturer). Thus, the theory grants us a new approach to program analysis and the ability to choose the programs complexity, while knowing the order of the approximations error.
\end{abstract}
...
For simplicity of expression we define the following functional spaces
...
Thus $\D^k$ is a mapping between functional spaces $\eqref{eq:F^n}$
 ...
 \begin{definicija}
 	Infinitely differentiable programming language $\dP$ is such a subspace of $\F$, that satisfies the relation
 ...
 \begin{proof}
 	 By $\eqref{eq:P}$ and the symmetric nature of the operator $\D$ for each $k$ there exists an isomorphism
 	 	\begin{equation}
 	 		\dP\to\D^k\dP
 	 	\end{equation}
 \end{proof}
 
 As a consequence of Theorem $\ref{izr:dP}$ the tuple $(\dP,\D)$ is a monoid. A fact we shal readily employ in our construction.
...
U\otimes T(V^*)$, composed of the map itself and all derivatives of order $n\le k$. The image $\sumd_kP(\x)$ is a multitensor of order $k$, which is a direct sum of the maps valueand all derivatives of order $n\le k$, all evaluated in the point $\x$:
\begin{equation}
  \label{eq:multi_odvod}
  \sumd_kP(\x) = P(\x)+\D_\x P + \D^2_\x P + \ldots + \D^k_\x P.
\end{equation}
\begin{izrek}\label{izr:tauRek}
  The operator $\sumd_k$ is recursively defined by the expression
  \begin{equation}
...
Let $\dP_n$ be the subspace, spanned by $\{\partial^n\}$ over $\dP$.
 ...
 \begin{izrek}\label{izr:P_n}
 	Functional space $\dP_n$ is isomorphic to the tensor product of the functional space $\dP_0:V\to V$ and tensor algebra $T(V^*)$ of the dual od the virtual space $V$.
 ...
 \begin{proof} 
 Induction hypothesis follows from $\eqref{eq:P}$.
 \begin{equation}
 	p\in\dP\iff \exists_{\II}\Big(\D^np=\sum\limits_{i\in\II} c_ip_i\otimes(V^*)^{n\otimes}\Big)
 \end{equation}
 Induction step by $\ref{izr:dP}$ follows from Theorem $\ref{izr:D^nRek}$ 
 $$\D p_1=p_2\in \dP\implies \D^{n+1}p_1=\D^np_2$$
  \end{proof}
   
   Consequentially, if there exists a construction o a first order programming language, there exists an implementations of a programming language of an arbitrary order. 
    ...
    The following
     ...
     \end{equation}
   	holds by Theorem $\ref{izr:P_n}$.
        \begin{opomba}
        Of course the same holds for all generating mappings of the type
        $$\dP\to\dP_n$$
        \end{opomba}
...
\end{equation}
 ...
 We motivate the following with an example. Suppose an algorithm of high time complexity. As speed is of importance, an approximation would suffice. Using the described mechanics, we may linearize the program. We choose a $v_0\in\dP$ and linearize $P'=P(v_0)+\D P(v-v_0)$. The latter has to be done only once. The approximation is of linear complexity $\mathcal{O}(k)$, where $k$ is the dimension of $V$.
 
 The image of the linearization is locally a good approximation of the original program. To extend this radius, we would like to expand said program in a series.By Theorem $\ref{izr:linearnaNeodvisnost}$  there exists $\dP_n$, that is spanned by the set $\{\D^k\}$ over $\dP$ (which is itself a functional space over $\RR$). Thus, the expression
...
As such $e^{h\D}$ is a mapping between functional spaces $\eqref{eq:F_n}$
...
Through $\eqref{eq:specProg}$ we may construct a map from the space of programs, to the space of polynomials. By $\eqref{eq:tenzor_algebra}$, $V_n$ is isomorphic to the commutative associative unitary algebra $S(V^{i*})$, which corresponds to polynomials with unknowns in $V^i$.
 \begin{equation}\label{eq:pToPol}
 	(e^{h\D})(V): \F\to S(V^*)
 \end{equation}
 The expression repressents a map from a program, to a polynomial. 
...
\begin{proof}
 ...
 \end{proof}       
 \begin{opomba}
 As such it may be convenient to apply $g(v)\in V$ to $f$ before hand, as $e^\D$ applied to a constant represents the identity map and grants us the derivative $\D^n$ at a point $v\in V$ instead of an expansion into a multitensor.
  ...
  \end{equation}
 \end{opomba}
 The Theorem $\ref{izr:kompo}$ enables a simple implementation of program composition in $\dP_n$, expressed as tensor series through the expression $\eqref{eq:dComposite}$. Yet there exists a simpler path towards such calculations on the level of operators. 
...
\frac{d}{dh}e^{h\D_g}=\D_ge^{h\D_g}
 ...
 As it is evident through the example, calculations using operators are far simpler, than direct manipulations of functional series, as was done in th proof of Theorem $\ref{izr:kompo}$. Which of course enables a simpler implementation, that functions over arbitrary function spaces. In the space that is spanned by $\{\D^n\}$ over $\dP$, derivatives of compositions may be expressed using just the product rule of Theorem $\ref{izr:prod}$, and $\eqref{eq:dexp}$ and $\eqref{eq:de}$, solely through the operators. Thus, explicit knowledge of rules for differentiating compositions is unnecessary, as it is contained in the structure of the operator $exp(\D_fe^{h\D_g})$ itself, which is differentiated using standard rules, as in the above example.
 ...
 \end{equation}
   ...
   \end{proof}
 We will readily employ the Theorem in usage.
   ...
   With technological advances, switching of the hardware is common, which may lead to  decline in performance of code. Thus, we would like to employ transformations of a program $P\in\dP$ in basis $F$. It is common to settle for a suboptimal algorithm, that is optimized for hardware $H$. Sub-optimality of the algorithm depends on the set $F$, whether it spans $P$, or not. A classic example of a transformation is the Fourier transform in the basis $\{sin(nx), cos(mx)\}$, that spans $\dP$ (which as stated is not true for an arbitrary set $F$).
  ...
  Using the tensor $\eqref{eq:matTrans}$, we can easily perform transformation of basis $\X\to F$. For a specific set $F$ (and consequentially a hardware $H$, upon which the set $F$ is conditioned to), the tensor $\eqref{eq:matTrans}$ only has to be computed once, and can be used for transforming arbitrary programs (while using the same operator $e^\D_n$).
  ...
  It makes sense to, before computing the tensor $\eqref{eq:matTrans}$, to expand the set $F$, by mutual (nested) compositions, and gain mappings, that can not be expressed as linear combinations (but are still optimized on hardvare $H$), and so increasing the power of the method.
 ...
 \begin{equation}
   ...
   \end{cases}
 \end{equation}
 Each control structure divides the space of parameters to different domains, in which the execution of the program is always the same. The entire program divides the space of all possible parameters to a finite set of domains $\{\Omega_i;\quad i=1,\ldots
 ...
 \begin{equation}
   ...
   \end{cases}
 \end{equation}
 Differential $T$ of program $P$, is of course dependent on initial parameters $\vec{x}$, and can also be expressed as a spline, inside domains $\Omega_i$
 \begin{equation}
 ...
 The problem is on the edge of domains $\partial\Omega_i$, where the program $P$ may not be differentiable. A problem which may be avoided, with methods of the coming sections.
  ...
  Now, we may inquire towards changes in programs iterations, as dependent on exit conditions. The answer would enable analysis and optimization of the number of iterations of the program. It also opens new doors in analysis of the Halting problem.
  
  Let $h$ be the map defined with eigen equation
  ...
  For example, using methods of gradient descent, we would use the velocity $\eqref{eq:hitrostIteracije}$, which would give the order and direction to the descent. Inductively, we could derive acceleration and higher order changes.
  
  Computation of he map $h$ and conditions for existence, for $p$ representable as a power series, was solved by Bridges (2011), derivation of which is omitted in this text, for the sake of brevity. By Theorem $\eqref{izr:e^d}$, we may expand each program $p\in\dP$ by applying the operator $e^\D$ into a power series and employed said computation.
  ...
  As a special case of iterator usage, we examine accumulators, for which we derive an explicit form, as a function of exit conditions. Let $\Shift^n$ denote the operator, that performs a linear shift of a program $p$ in the direction $v$.
   ...
   By Theorem $\ref{izr:linearnaNeodvisnost}$  there exists a space $\dP_n$, spanned by $\{\D^k\}$ over $\RR$. Thus the expression   
     
    \begin{equation}
    	\frac{h\D}{1-e^{h\D}}=\sum\limits_{n=0}^{\infty}c_n\frac{(h\D)^n}{n!}
    \end{equation}
    is well defined. It turns out that $c_i=B_i$, with $B_i$ being the $i$-th Bernuli number. The operator $(1-\Shift^n)$ represents evaluation at endpoints (exit conditions).
...
Branching of the program into domains is done through conditional statements. The number of domain $\Omega_i$ equals cardinality of the set $\{\Omega_i\}$. Each conditional causes a branching in programs' execution tree.

\begin{izrek}\label{izr:st.zlepkov}
Cardniality of the set $\{\Omega_i\}$ equals $\lvert\{\Omega_i \}\rvert=2^k$, where $k$ is the number of conditionals contained within the program.
\end{izrek}
\begin{opomba}
...
\begin{proof}
	...
	Thus, the source code contains $2n$ differentiable braches, from first branching on, not counting the branch leading up to it, upon which the application of the operator $e^\D$ is need. Total of $2n+1$. By Theorem $\ref{izr:P_n}$, each of these branches is analytic.
\end{proof}

Images of the operator $e^\D$ and $T_{F\X}$  are elements of the original space $\dP$, which may be composed. Thus, for $P=p_3\circ p_2\circ p_1$, the folowing makes sense

\begin{equation}
P=\Big(p_3\circ e^ \D(p_2)\circ T_{F\X}e^\D(p_1)\Big) \in \dP
\end{equation}

Of course the same holds for all permutations of aplications of operators $e^\D$, $T_{F\X}$ and $id$, as visible in Figure $\ref{fig:diagram}$.

\begin{opomba}
Equality depends on the set $F$, specified by the manufacturer of the hardware.
\end{opomba}

The transformation tensor $T_{F\X}$ is needed to be computed only once and can be applied to all programs running on said hardware. The same holds true for each branch $p_i$, which can, by Theorem $\ref{izr:komp_homo}$, be freely composed between them.

\begin{izrek}
Composition of images of the eoperator $e_n^\D(p_2)\circ e_n^\D(p_1)$ is computable in $\mathcal{O}(nlog^k(n))$, with $k$ being the dimension of $V$ and $n$ the number of basis vectors $\{\D^n\}$ upon which we are projecting.
\end{izrek}
