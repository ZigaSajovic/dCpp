...
For simplicity of expression we define the following function spaces
...
Thus $\D^k$ is a mapping between function spaces $\eqref{eq:F^n}$
 ...
 \begin{definicija}
 	Infinitely differentiable programming language $\dP$ is any subspace of $\F$ satisfying the relation
...
\begin{corollary}
The tuple $(\dP,\D)$ is a monoid, a property to be readily employed.
\end{corollary}
...
U\otimes T(V^*)$, composed of the map itself and all derivatives of order $n\le k$. The image $\sumd_kP(\x)$ is a multitensor of order $k$, which is a direct sum of the maps value and all derivatives of order $n\le k$, all evaluated in the point $\x$:
\begin{equation}
...
Let $\dP_n$ be the subspace, spanned by $\{\partial^n\}$ over $\dP$.
 ...
 $$\D p_1=p_2\in \dP\implies \D^{n+1}p_1=\D^np_2$$
  \end{proof}
  \begin{corollary}
  If there exists a construction of a first order programming language $\dP_1$, there exists an implementations of a programming language of an arbitrary order $\dP_n$.
  \begin{equation}
