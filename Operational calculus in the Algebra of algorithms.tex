 ...
 \begin{proof} Follows directly from the proof of Theorems $\ref{izr:P}$ and $\ref{izr:dP}$ through argument $\eqref{eq:inductionStep}$.
  ...
  \end{corollary}
      \begin{corollary}\label{rem:vTen}
       Maps $V\otimes T(V^*)\to V\otimes T(V^*)$ are constructible through tensor algebra on $\dP_n$ and through it, compositions of programs in $\dP_n$. As such, it is assumed so heron.
       \end{corollary}
...
An infinte-differentiable programming space $\dP_n$ is a function algebra, with the product defined by $\eqref{eq:P_prod}$.
\end{izrek}

\subsection{Analytic programming language}
